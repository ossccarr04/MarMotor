    ...
    this.authService.register(userRegister).subscribe({
      ...
      error: (err) => {
        ...
        if (err.status === 403 || err.error?.code === environment.PROTECTED_EMAILS) {
          this.toast.error('No se ha podido completar el registro con los datos proporcionados. Si ya tienes una cuenta, intenta iniciar sesión.', 'Error de Seguridad');
          this.registerForm.get('email')?.setErrors({ emailProtegido: true });
        } else if (err.status === 409) {
          this.toast.warning('No se ha podido completar el registro con los datos proporcionados. Si ya tienes una cuenta, intenta iniciar sesión.', 'Aviso');
        } else {
