    ...
    this.registerForm = this.fb.group(
      {
        ...
        email: [
          '',
          [Validators.required, Validators.email, this.emailProtegidoValidator.bind(this)],
        ],
        password: [
          '',
          [Validators.required, Validators.minLength(8), Validators.pattern(this.regexPassword)],
        ],
      ...
      this.authService.register(userRegister).subscribe({
        ...
        error: (err) => {
          if (err.status === 403 || err.error?.code === environment.PROTECTED_EMAILS) {
            this.toast.error(
              'No se ha podido completar el registro con los datos proporcionados. Si ya tienes una cuenta, intenta iniciar sesión.',
              'Error de Seguridad',
            );
